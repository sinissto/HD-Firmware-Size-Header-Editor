...
function StatusBanner({ result }: { result: BatchResult }): JSX.Element {
  const successCount = result.files.filter((f) => f.status === 'success').length
  ...
  if (result.status === 'success') {
    return (
      <div className="mt-6 rounded-lg border border-green-700 bg-green-950 p-4 text-sm">
        ...
        <p className="break-all font-mono text-xs text-gray-400">{result.editedFolderPath}</p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="flex-1 break-all font-mono text-gray-200">{file.fileName}</span>
      </div>
      {isSuccess && file.size !== undefined && file.headerValue !== undefined && (
        <dl className="mt-2 space-y-1 text-gray-400">
          <div className="flex justify-between gap-4">
            <dt>File size</dt>
            <dd className="font-mono text-gray-300">{file.size.toLocaleString()} bytes</dd>
          </div>
          <div className="flex justify-between gap-4">
            <dt>Header value</dt>
        ...
        <>
          ...
          {result.editedFolderPath && (
            <p className="mt-4 break-all text-xs text-gray-500">
              ...
              <span className="font-mono text-gray-400">{result.editedFolderPath}</span>
            </p>
          )}
        </>
      )}
    ...
    <>
      ...
      <div className="flex-1">
        <p className="mb-6 text-sm text-gray-400">
          ...
          <code className="rounded bg-gray-800 px-1">.bin</code> files. A copy named{' '}
          <code className="rounded bg-gray-800 px-1">&lt;folderName&gt;_EDITED</code>{' '}
          will be created alongside the original and all{' '}
          <code className="rounded bg-gray-800 px-1">.bin</code> files inside will
          have their headers written. Your original files are never modified.
        </p>
      </div>
